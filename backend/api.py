...
async def analyze_instagram(request: AnalyzeRequest, token: str = Depends(verify_token)):
    ...
    try:
        ...
        # Get result from database — retry up to 6 times in case the SQLite WAL
        # hasn't made the subprocess write visible to this process yet.
        # check_cache() now uses a fresh connection each time to avoid stale snapshots.
        analysis = None
        for _attempt in range(6):
            ...
            if analysis:
                ...
                break
            if _attempt < 5:
                logger.warning(f"⏳ [{shortcode}] Not in DB yet (attempt {_attempt+1}/6), retrying in 1s…")
                await asyncio.sleep(1)
        
        if not analysis:
            logger.error(f"❌ [{shortcode}] Not found in database after 6 attempts!")
            raise HTTPException(
                status_code=500,
